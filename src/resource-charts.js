...
const fieldLabel=f=>({budget:'Planned',actual:'Actual',remaining:'Remaining',forecast:'Forecast'})[f]||f;
export function renderHistogram(el,rows,{fields=['budget','actual','remaining'],title='Resource Histogram'}={}){
  ...
  rows.forEach((r,i)=>{fields.forEach((f,j)=>{const v=Number(r[f])||0,h=v/max*plotH,x=pad+i*group+2+j*barW,y=H-pad-h;svg+=`<rect x="${x}" y="${y}" width="${Math.max(2,barW-1)}" height="${h}" class="bar-${f}"><title>${esc(r.date)} ${esc(fieldLabel(f))}: ${v.toFixed(2)}</title></rect>`});if(i%Math.max(1,Math.ceil(rows.length/12))===0)svg+=`<text x="${pad+i*group}" y="${H-12}" class="axis-label">${esc(r.date.slice(5))}</text>`});svg+='</svg>';el.innerHTML=svg;
}
